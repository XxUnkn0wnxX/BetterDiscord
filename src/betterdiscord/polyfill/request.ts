...
function fixBuffer(options: RequestOptions, callback: RequestCallback): RequestCallback {
    return (error, res, body) => {
        try {
            ...
            if (String(contentType) !== "text/plain") {
                body = Buffer.from(body as any) as any;
            }
            else {
                body = Buffer.from(body as any).toString();
            }
        }
        catch (err) {
