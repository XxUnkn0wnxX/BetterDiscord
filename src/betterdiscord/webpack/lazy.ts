...
/** @internal */
export function getOrInsertComputedCompat<K, V>(map: Map<K, V>, key: K, callback: (key: K) => V): V {
    const nativeInsert = (map as MapWithComputedInsert<K, V>).getOrInsertComputed;
    if (typeof nativeInsert === "function") return nativeInsert.call(map, key, callback);
...
const ContentCache = new Map<string, Promise<boolean>>();
...
export async function loadEntry(string: NOOP | string) {
    ...
    await Promise.all(chunks.map(async ([, rawChunkIds, entryPoint]) => {
        ...
        await Promise.all(chunkIds.map(async id => {
            ...
            try {
                const isWorker = await getOrInsertComputedCompat(ContentCache, id, () => {
