...
| Plugin/theme settings, search, and editors during hot reload | Upstream refreshes the addon list but leaves settings panels and BetterDiscord editor windows created from the old addon open. Its installed-addon search also stores the visible text separately from the filter and labels the placeholder with the filtered result count. | Closes only the matching settings modal and BetterDiscord detached/external source editors, using a discard-only path with no toast, prompt, automatic reopen, or BetterDiscord save callback. The installed plugin/theme search keeps one controlled query across header remounts, labels its placeholder with the full installed count, and reserves the filtered count for the results label. Normal user closes keep their existing behavior; system-editor processes remain untouched. | Preserve the addon type/ID/filename-scoped reload close and controlled installed-list search. Do not replace them with a global modal/window close, make reload invoke normal save/confirm callbacks, or use the filtered result count as the installed-total placeholder. |
...
- The installed plugin/theme search value is controlled by the addon page so a
  ...
  leaving a hidden filter active. Its placeholder always reports the full
  installed count; only the title's results label reports the filtered count.
  The Addon Store search remains unchanged.
