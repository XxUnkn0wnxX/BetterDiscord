...
The reviewed `37a229a5..dfb4148f` range (`6290302d`, `0516531d`, and merge
`dfb4148f`) produced a synthetic conflict in `number.tsx`. The fork side wins
the surrounding implementation because the upstream Number decrement min-guard
behavior is already present there, while the fork's controlled/uncontrolled,
finite-input, provider/local-disabled, and disabled-button contracts remain
intact; no source adaptation was needed. The focused Number suite passed twice,
each with 6 tests, 20 expectations, and 0 failures. `bun run typecheck` passed,
and the full suite passed with 459 passes, 24 expected skips, 0 failures, and
1,268 expectations across 55 files. No local build was run because the source
...
- Color controls use an explicit `defaultColor` as the reset target, including
  ...
  initial state only.
- `buildSettingsPanel` invokes an individual setting callback before its panel
  callback, exactly once each. Root controls use the builder wrapper for panel
  dispatch; category controls leave panel dispatch to `Group` rather than
  adding a second callback inside each child wrapper.
